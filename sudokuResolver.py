...
# You just need to change the digits in the global variable and run the script to complete your sudoku. Each list represents a line.
...
def alreadyExistsInRow(grid, number, position):
    """" Function that determines if a number already exists in the row containing the current box, for the entire grid. """
    row = (position // 9) # Every 9 boxes we go to the next line.
    row = (row - 1) if (position % 9 == 0) else row # If position % 9 = 0, the row is too great by 1.
    return number in grid[row]

def alreadyExistsInColumn(grid, number, position):
    """" Function that determines if a number already exists in the column containing the current box, for the entire grid. """
...
def alreadyExistsInArea(grid, number, position):
    """ Function that determines if a number already exists in the sub-grid containing the current box. """
    row = (position // 9)
    row = (row - 1) if (position % 9 == 0) else row # If position % 9 = 0, the row is too great by 1.
...
def completeSudoku(grid, position):
    ...
    if (grid[row][column] != 0): # If the current box is already completed.
    ...
    for i in range(1, 10): # Every possible digit that we can write in a box.
        if (canBeWritten(grid, i, position)):
            grid[row][column] = i # We try this digit.
            if(completeSudoku(grid, position + 1)): # We try to solve the sudoku with the current choice, if it's impossible, we go back.
                return True # If the grid is fully solved.
        

    grid[row][column] = 0 # Reset the current box, the choice wasn't good.
    return False
